        ...
        <div style={{ backgroundColor: '#1a1a1a', borderRadius: 8, padding: 20 }}>
            ...
            {result && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    {/* Aggregated metrics - LLM Judge */}
                    <div>
                        <h4 style={{ margin: '0 0 12px 0', color: '#8b5cf6', fontSize: 16 }}>
                            LLM Judge Detection
                        </h4>
                        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                            <AggregatedMetricsCard
                                title="RAG (LLM Judge)"
                                metrics={result.aggregated.llmJudge.rag}
                                variant="rag"
                            />
                            <AggregatedMetricsCard
                                title="NoRAG (LLM Judge)"
                                metrics={result.aggregated.llmJudge.noRag}
                                variant="norag"
                            />
                            <AggregatedDeltaCard
                                title="LLM Judge Δ"
                                deltas={result.aggregated.llmJudge.deltas}
                            />
                        </div>
                    </div>

                    {/* Aggregated metrics - NLI */}
                    <div>
                        <h4 style={{ margin: '0 0 12px 0', color: '#ec4899', fontSize: 16 }}>
                            NLI Detection
                        </h4>
                        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                            <AggregatedMetricsCard
                                title="RAG (NLI)"
                                metrics={result.aggregated.nli.rag}
                                variant="rag"
                            />
                            <AggregatedMetricsCard
                                title="NoRAG (NLI)"
                                metrics={result.aggregated.nli.noRag}
                                variant="norag"
                            />
                            <AggregatedDeltaCard title="NLI Δ" deltas={result.aggregated.nli.deltas} />
                        </div>
                    </div>
    ...
    return (
        <div style={{ backgroundColor: '#252525', borderRadius: 6, overflow: 'hidden' }}>
            ...
            >
                ...
                <div>
                    <div style={{ color: '#8b5cf6', fontSize: 11, marginBottom: 2 }}>LLM Judge</div>
                    <div style={{ display: 'flex', gap: 8, fontSize: 12 }}>
                        <span style={{ color: '#10b981' }}>
                            RAG: {(paper.rag.llmJudgeMetrics.hallucinationRate * 100).toFixed(1)}%
                        </span>
                        <span style={{ color: '#f59e0b' }}>
                            NoRAG: {(paper.noRag.llmJudgeMetrics.hallucinationRate * 100).toFixed(1)}%
                        </span>
                    </div>
                </div>
                <div>
                    <div style={{ color: '#ec4899', fontSize: 11, marginBottom: 2 }}>NLI</div>
                    <div style={{ display: 'flex', gap: 8, fontSize: 12 }}>
                        <span style={{ color: '#10b981' }}>
                            RAG: {(paper.rag.nliMetrics.hallucinationRate * 100).toFixed(1)}%
                        </span>
                        <span style={{ color: '#f59e0b' }}>
                            NoRAG: {(paper.noRag.nliMetrics.hallucinationRate * 100).toFixed(1)}%
                        </span>
                    </div>
                </div>
...
function ClaimItem({ claim, index }: { claim: ClaimAnalysis; index: number }) {
    ...
    return (
        ...
        >
            ...
            <div style={{ color: '#ddd', marginBottom: 8 }}>{claim.text}</div>
            ...
            <div style={{ paddingLeft: 8, borderLeft: `2px solid ${nliColor}` }}>
                <div style={{ color: '#ec4899', fontSize: 10, fontWeight: 600, marginBottom: 2 }}>
                    NLI (E: {(claim.nli.scores.entailment * 100).toFixed(0)}% | N: {(claim.nli.scores.neutral * 100).toFixed(0)}% | C: {(claim.nli.scores.contradiction * 100).toFixed(0)}%)
                </div>
            </div>
        </div>
    );
